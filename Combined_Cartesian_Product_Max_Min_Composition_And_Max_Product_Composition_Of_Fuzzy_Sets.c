...
// Function to compute Max-Min Composition of two fuzzy relations
FuzzyRelation maxMinComposition(FuzzyRelation R, FuzzyRelation S) {
    printf("\n=== COMPUTING MAX-MIN COMPOSITION ===\n");
    ...
    for (int i = 0; i < T.rows; i++) {
        for (int j = 0; j < T.cols; j++) {
            ...
            for (int k = 0; k < R.cols; k++) {
                // Calculate min(R[i,k], S[k,j])
                float min_val = R.matrix[i][k] < S.matrix[k][j] ? R.matrix[i][k] : S.matrix[k][j];

                printf("    - min(R[%d,%d]=%.2f, S[%d,%d]=%.2f) = %.2f\n",
                       i, k, R.matrix[i][k], k, j, S.matrix[k][j], min_val);

                // Update T[i,j] if this min value is greater
                if (min_val > T.matrix[i][j]) {
                    T.matrix[i][j] = min_val;
    ...
    printf("\n[CALCULATION COMPLETE] Max-Min composition computed successfully\n");
    return T;
}
...
// Function to explain Max-Min Composition theory
void explainMaxMinComposition() {
    printf("\n======================================\n");
    printf("THEORY: MAX-MIN COMPOSITION OF FUZZY RELATIONS\n");
    printf("======================================\n");
    ...
    printf("The Max-Min Composition T = R∘S from X to Z is defined as:\n");
    printf("T(x,z) = max{min[R(x,y), S(y,z)]} for all y in Y\n\n");

    printf("Where:\n");
    printf("- T(x,z) is the membership degree of the pair (x,z) in the resulting relation\n");
    printf("- For each intermediate element y, we find min[R(x,y), S(y,z)]\n");
    printf("- Then take the maximum of all these minimum values\n");
    printf("======================================\n\n");
    ...
    printf("   - Calculate min(R[i,k], S[k,j]) for each k from 0 to n-1\n");
    printf("   - T[i,j] is the maximum of all these minimum values\n");
    printf("======================================\n\n");
}
...
int main() {
    int choice, compositionType;

    // Display welcome message and theoretical explanation
    printf("==========================================================\n");
    printf("FUZZY SET OPERATIONS: CARTESIAN PRODUCT & FUZZY COMPOSITIONS\n");
    printf("==========================================================\n");
    ...
    printf("2. Composition of two Fuzzy Relations\n");
    printf("3. Complete Example (create sets, compute products, then composition)\n");
    ...
    if (choice == 1 || choice == 3) {
        ...
        if (choice == 3) {
            // For choice 3, continue with composition
            ...
            // Ask for composition type
            printf("\nSelect composition type:\n");
            printf("1. Max-Min Composition\n");
            printf("2. Max-Product Composition\n");
            printf("3. Both Max-Min and Max-Product Compositions\n");
            printf("Enter your choice (1-3): ");
            scanf("%d", &compositionType);

            // Validate choice
            while (compositionType < 1 || compositionType > 3) {
                printf("Invalid choice. Please enter 1, 2, or 3: ");
                scanf("%d", &compositionType);
            }

            if (compositionType == 1 || compositionType == 3) {
                // Explain Max-Min Composition theory
                explainMaxMinComposition();

                // Compute Max-Min Composition
                FuzzyRelation T_min = maxMinComposition(R, S);

                // Print the Max-Min composition result
                printRelationMatrix(T_min, "T (R∘S) Max-Min");

                // Free the memory
                freeRelation(T_min);
            }

            if (compositionType == 2 || compositionType == 3) {
                ...
                printRelationMatrix(T_product, "T (R∘S) Max-Product");
        ...
        // Explain composition theories
        explainMaxMinComposition();
        explainMaxProductComposition();

        // For simplicity, we'll create two relation matrices directly
        printf("\n=== CREATING TWO FUZZY RELATION MATRICES ===\n");
        printf("For this example, we'll create two fuzzy relation matrices manually.\n");

        // Get dimensions for the first relation R
        int r_rows, r_cols;
        printf("\nEnter dimensions for the first relation R:\n");
        printf("Number of rows: ");
        scanf("%d", &r_rows);
        printf("Number of columns: ");
        scanf("%d", &r_cols);

        // Create the first relation R
        FuzzyRelation R;
        R.rows = r_rows;
        R.cols = r_cols;

        // Allocate memory
        R.matrix = (float**)malloc(R.rows * sizeof(float*));
        R.xValues = (float*)malloc(R.rows * sizeof(float));
        R.yValues = (float*)malloc(R.cols * sizeof(float));

        for (int i = 0; i < R.rows; i++) {
            R.matrix[i] = (float*)malloc(R.cols * sizeof(float));
        }

        // Input values for R
        printf("\nEnter values for relation R:\n");

        // Input x universe values
        printf("Enter X universe values:\n");
        for (int i = 0; i < R.rows; i++) {
            printf("X[%d]: ", i);
            scanf("%f", &R.xValues[i]);
        }

        // Input y universe values
        printf("Enter Y universe values:\n");
        ...
        printf("Enter membership values for relation R:\n");
        for (int i = 0; i < R.rows; i++) {
            for (int j = 0; j < R.cols; j++) {
                printf("R[%d,%d] (between %.2f and %.2f): ", i, j, R.xValues[i], R.yValues[j]);
                scanf("%f", &R.matrix[i][j]);

                // Validate membership value
                while (R.matrix[i][j] < 0 || R.matrix[i][j] > 1) {
                    printf("Membership value must be between 0 and 1. Enter again: ");
                    scanf("%f", &R.matrix[i][j]);
                }
            }
        }
        ...
        // Get dimensions for the second relation S
        int s_rows, s_cols;
        printf("\nEnter dimensions for the second relation S:\n");
        printf("Number of rows must be %d (same as columns of R): ", R.cols);
        s_rows = R.cols;
        printf("%d\n", s_rows);
        printf("Number of columns: ");
        scanf("%d", &s_cols);

        // Create the second relation S
        FuzzyRelation S;
        S.rows = s_rows;
        S.cols = s_cols;

        // Allocate memory
        S.matrix = (float**)malloc(S.rows * sizeof(float*));
        S.xValues = (float*)malloc(S.rows * sizeof(float));
        S.yValues = (float*)malloc(S.cols * sizeof(float));

        for (int i = 0; i < S.rows; i++) {
            S.matrix[i] = (float*)malloc(S.cols * sizeof(float));
        }

        // Copy Y values from R to X values of S
        for (int i = 0; i < S.rows; i++) {
            S.xValues[i] = R.yValues[i];
        }

        // Input z universe values
        printf("Enter Z universe values:\n");
        ...
        // Input membership values
        printf("Enter membership values for relation S:\n");
        for (int i = 0; i < S.rows; i++) {
            for (int j = 0; j < S.cols; j++) {
                printf("S[%d,%d] (between %.2f and %.2f): ", i, j, S.xValues[i], S.yValues[j]);
    ...
    printf("Thank you for using the Fuzzy Set Operations program!\n");
